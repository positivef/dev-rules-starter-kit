...
def create_checkpoint():
    ...
    try:
        print("\n💾 Creating checkpoint (git stash)...")
        subprocess.run(
            ["git", "stash", "push", "-u", "-m", "dev-rules-setup-checkpoint"],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        print("   ⚠️  No git repository or nothing to stash", file=sys.stderr)
        return False
    except FileNotFoundError:
        print("   ⚠️  Git not found in PATH", file=sys.stderr)
        return False
